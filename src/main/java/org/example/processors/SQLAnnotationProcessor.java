...
import org.example.anotacii.Stovpec;
import org.example.anotacii.Klyuch;
import org.example.anotacii.Tablycya;
...
@SupportedAnnotationTypes({"org.example.anotacii.Tablycya"})
@SupportedSourceVersion(SourceVersion.RELEASE_17)
public class SQLAnnotationProcessor extends AbstractProcessor {

    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (Element element : roundEnv.getElementsAnnotatedWith(Tablycya.class)) {
    ...
    private void processTable(TypeElement classElement) {
        Tablycya tableAnnotation = classElement.getAnnotation(Tablycya.class);
        String tableName = tableAnnotation.nazva();
        ...
        for (Element field : classElement.getEnclosedElements()) {
            if (field.getKind() == ElementKind.FIELD && field.getAnnotation(Stovpec.class) != null) {
                Stovpec columnAnnotation = field.getAnnotation(Stovpec.class);
                String columnName = columnAnnotation.nazva();
                String columnType = getSQLType(field.asType());

                if (field.getAnnotation(Klyuch.class) != null) {
    ...
    private void generateSQLGeneratorClass(TypeElement classElement, String tableName, String createQuery, String insertQuery, String updateQuery, String deleteQuery, String fieldsMapping) {
        ...
        if (insertParams.length() > 0) {
            insertParams.setLength(insertParams.length() - 2);
            insertValues.setLength(insertValues.length() - 2);
            selectColumns.setLength(selectColumns.length() - 2);
        }
